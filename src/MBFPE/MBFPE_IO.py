...
This module provide the basic reading and writing functions for BPFPE
...
import time
import pandas as pd
from .MBFPE_utils import *
from .MBFPE_SPACE_TIME import *
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 	
...
def get_FILDA_size(filename, verbose=0):
	...
	printf(message, verbose, prefix = 'MBFPE_IO')
...
def read_data(nl, tt):
	...
	for op in ops:
		...
		if len(file_temp) > 0:
			...
			printf(message, 1, prefix = 'MBFPE_IO')
...
def dict2nc(save_dict, nl, sat, data_type, savename):
	...
	ncid.LongName = 'MBFPE fire parameter estimation on FILDA ' + metadata_dict['satellite'] + '47MOD.'
	ncid.history = "Created " + time.ctime(time.time()) + ' ' + sys.argv[0]
