'''
This is the main library of Multichannel Biphasic Fire Parameter
Estimation (BPFPE)
...
BPFPE dependents on the following python libraries:
...
#-----------------------------------------------------------------------
# inport the build libraries used in BPFPE
...
try:
    ...
    import theano.tensor as tensor
    import pymc3 as pm
    print(" - MBFPE: Imported theano with pymc3")
    ...
    try:
        ...
        import pymc as pm
        print(" - MBFPE: Imported pytensor with pymc")
    except ImportError:
...
from . import MBFPE_IO as IO # module for reading and writing files
from . import MBFPE_SPACE_TIME as ST # module 
from . import MBFPE_SENSOR as SR # module for 
from . import MBFPE_NAMELIST as NL # module for 
from . import MBFPE_MODEL_TENSOR as MT # module for
from . import MBFPE_EVAL as EV # module for 
from .MBFPE_utils import printf
...
def init_uniphasic_estimation(nl, fss):
	...
	# Placeholders means it will only be used to create the computation 
	# graph, Those values will be updated for each real case
	printf(f'Define tensor for Uniphasic estimator...', 1, prefix = 'MBFPE')
	...
	with pm.Model() as uniphase_estimator:
		...
		if is_pymc3:
			...
			# Prior for 't_mean'
			t_mean = PositiveNormal_T('t_mean', mu=nl.mean_ts, sigma=nl.sigma_ts)
			# Prior for 'p_f'
			p_mean = PositiveNormal_P('p_mean', mu=nl.mean_pf, sigma=nl.sigma_pf)  
		else:
			t_mean = pm.TruncatedNormal('t_mean', mu=nl.mean_ts, sigma=nl.sigma_ts, lower=300, upper = 2200)
			# Prior for 'p_f'
			p_mean = pm.TruncatedNormal('p_mean', mu=nl.mean_pf, sigma=nl.sigma_pf, lower=0.0)
		# well, I have to use a weird line breaking, to make the line 
		# shorter for reading
...
def estimate_bg(bg_estimator, nl, ts_bg, bg_obs, verbose = 0):
	...
	printf(message, verbose, prefix = 'MBFPE')
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
def MBFFP_MP(nl, data_bg, data_fire, 
             bg_estimator, biphase_estimator, uniphase_estimator,
             ts_bg, ts_bi_fire, ts_uni_fire,
             bgs, fss):
...
def get_state_output(array_dict, filda_dict):
	...
	for i in range(n_fire):
		...
		if (QA_flag == FLAG_UNIPHASIC) | (QA_flag == FLAG_DEDGRADATE) | \
			...
			state_params = ['t_s', 'p_s', 'FP_Power_T']  
			for param in state_params:
	...
	FP_conbined_land_type = copy.deepcopy(state_dict['FP_Land_Type'])
	
	idx = np.where(state_dict['Static_flag'] == 1)
	FP_conbined_land_type[idx] = 101
	
	idx = np.where(state_dict['FP_Gas_Flaring'] == 1)
	FP_conbined_land_type[idx] = 102
	
	state_dict['FP_conbined_land_type'] = FP_conbined_land_type
	...
	for i in range(len(result)):
		...
		if qa >= 100:
			...
			if (qa == FLAG_UNIPHASIC) | (qa == FLAG_DEDGRADATE) | \
				...
				array_dict['t_s'][i, :] = result[i]['trace']['t_mean']
				array_dict['p_s'][i, :] = result[i]['trace']['p_mean']
